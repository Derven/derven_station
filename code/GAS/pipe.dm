	...
	proc/PROCESSING(var/list/L)
		spawn while(1)
			...
			for(var/obj/machinery/atmospherics/air_pipe/AP in L)
				if(world.cpu > 40)
					sleep(1)
				if(!istype(AP,/obj/machinery/atmospherics/air_pipe/pipe/inner))
					if(!istype(AP,/obj/machinery/atmospherics/air_pipe/pipe/outer))
						if(AP.oxy > 0 || AP.nit > 0 || AP.plas > 0)
							if(world.cpu > 60)
								sleep(2)
							AP.process()
				if(istype(AP,/obj/machinery/atmospherics/air_pipe/pipe/connector))
					AP.process()
	New(var/list/L)
		PROCESSING(L)
